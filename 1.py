...
ALL_LABELS_COUNT_FILE = "labelCount.txt"
...
def add_final_training_ops(class_count, weight_per_class, final_tensor_name, bottleneck_tensor):
    ...
    with tf.name_scope('cross_entropy'):
        weigth_per_class = tf.constant(weight_per_class)
        cross_entropy = tf.nn.weighted_cross_entropy_with_logits(logits=logits, targets=ground_truth_input,
                                                                 pos_weight=weigth_per_class)
...
def export_model(sess, labels, saved_model_dir, architecture="inception_v3"):
  ...
  """
    ...
    outputs = {
        ...
        tf.saved_model.signature_constants.CLASSIFY_OUTPUT_CLASSES:
            tf.saved_model.utils.build_tensor_info(tf.convert_to_tensor(labels))
    }
...
def generate_all_labels_txt():
    labels = []
    number = 0
    for file in glob.glob('%s/*.txt' % IMAGE_LABELS_DIR):
        number += 1
        with open(file, "r") as myfile:
            data = myfile.read().splitlines()
            labels.extend(data)
    counter = collections.Counter(labels)
    with open(ALL_LABELS_COUNT_FILE, "w") as f:
        f.write(simplejson.dumps(OrderedDict(counter)))
...
def main(_):
    ...
    number_of_images = generate_all_labels_txt()
    ...
    if not os.path.isfile(ALL_LABELS_COUNT_FILE):
        print('File {} containing all possible labels (= classes) does not exist.\n'
              ...
              'that is in all the possible classes.'.format(ALL_LABELS_COUNT_FILE))
        return -1

    with open(ALL_LABELS_COUNT_FILE) as f:
        content = simplejson.load(f)
        weight_per_class = []
        labels = []
        for i in content:
            weights = number_of_images / content.get(i)
            weight_per_class.append(weights)
            labels.append(i)
        class_count = len(weight_per_class)
    ...
    (train_step, cross_entropy, bottleneck_input, ground_truth_input,
     final_tensor) = add_final_training_ops(class_count, weight_per_class,
    ...
    with gfile.FastGFile(FLAGS.output_labels, 'w') as f:
        ...
        export_model(sess, labels, FLAGS.saved_model_dir)
